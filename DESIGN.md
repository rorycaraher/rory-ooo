...
- One centered column (max 1025px), no sidebars, no dense multi-column layouts outside the Projects card grid.
...
### Primary
- **Active Violet** (`#b175d6`): The system's only accent. Used for link text (default state, not hover), the active nav item's label and underline, `h1` heading color, `<strong>` emphasis inside list items, and the border color a project card shifts to on hover/focus. It never appears as a background fill or decoration — only as text/border color marking something live or important.
...
- **Stone Grey** (`#a7a7a7`): Secondary text — card descriptions, footer copy, default (non-hover, non-active) nav link color.
- **Graphite** (`#4e4e4e`): The only border color in the system — project card borders, the footer's top rule.
...
- **Title** (300, `clamp(1.8rem, 4.5vw, 2.8rem)` / 18–28px, line-height 1.3, letter-spacing -0.1rem): `h3` — project card titles, section subheadings.
- **Body** (300, 1.6rem/16px, line-height 1.6, fixed — no fluid scaling): all running text, including card descriptions (in Stone Grey) and footer copy (in Stone Grey, 1.4rem).
...
A single centered column, `max-width: 1025px`, with `1rem` horizontal padding (`.mg-container`) — no sidebars, no asymmetric or multi-column page layouts outside the Projects grid. The type scale is rem-based on a `62.5%` root font-size (so `1rem = 10px`), which keeps all sizing in the CSS legible as near-pixel values.

The one grid in the system is the Projects card grid (`.mg-row.card-grid`): full-width single column on mobile (`.mg-x12`), three-up (`.mg-m4`, 33.33% each) above 769px. Vertical rhythm is coarse and consistent: `3.2rem` above major sections, `6.4rem` above the footer, `1.6rem`/`2rem` between smaller elements. There is no dense or compact mode — spacing stays generous throughout.
...
Flat by design — there are no shadows anywhere in the system. Depth and state are conveyed entirely through border-color and text-color shifts (Graphite → Active Violet on card hover; Stone Grey → Active Violet on active nav), not through layering, blur, or lift.
...
Minimal, understated rounding: a single `0.4rem` (4px) radius on project cards, applied nowhere else. Borders are hairline (`0.1rem`/1px, Graphite) and are the system's primary form language — nav uses a `0.2rem` bottom border as an underline indicator for the active item rather than a filled or pill-shaped tab. No pill shapes, no heavy rounding, no clipping or asymmetric geometry anywhere.
...
### Cards / Containers
- **Corner Style:** `0.4rem` radius.
- **Background:** none — inherits the page's Charcoal Ink; cards are defined by their border, not a filled surface.
- **Border:** `0.1rem` solid Graphite at rest, shifting to Active Violet on hover/focus-visible (`border-color 0.15s ease`).
- **Shadow Strategy:** none — see Elevation & Depth.
- **Internal Padding:** `2rem 2.4rem`.
- **Behavior:** the entire card is the link (project name + description); title is Title-scale, description is Body-scale in Stone Grey.
...
- **Do** keep the page in a single centered column (`max-width: 1025px`) outside the Projects card grid.

### Don't:
- **Don't** add `box-shadow` anywhere — depth is conveyed by border-color and text-color only.
- **Don't** fill cards, nav items, or buttons with a background color on hover or active state — every existing state change is a border/text color shift, never a fill.
- **Don't** introduce bold or semi-bold weights for emphasis; use Active Violet color on `<strong>` instead.
- **Don't** round corners beyond `0.4rem` or introduce pill shapes — the system's rounding is deliberately subtle.
